...
describe("cover letter API contract", () => {
  it("accepts valid input and applies defaults", () => {
    ...
    expect(parsed.useAiGeneratedContent).toBe(false);
  });

  it("accepts an explicit AI generation flag", () => {
    const parsed = GenerateCoverLetterBody.parse({
      ...validInput,
      useAiGeneratedContent: true,
    });

    expect(parsed.useAiGeneratedContent).toBe(true);
  });
