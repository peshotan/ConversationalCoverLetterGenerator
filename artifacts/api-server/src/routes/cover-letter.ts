...
router.post("/cover-letter/generate", async (req, res) => {
  ...
  if (!input.useAiGeneratedContent) {
    const resumeText = input.resumeText;
    if (!resumeText?.trim()) {
      res.status(400).json({ error: "Paste your resume text or turn on Use AI-generated content to draft from a PDF." });
      return;
    }
    ...
    req.log.info(
      ...
      "Generated deterministic cover letter draft",
    );
    res.json(result);
    return;
  }
