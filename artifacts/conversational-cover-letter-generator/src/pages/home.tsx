...
export default function Home() {
  ...
  const hasResumeSource = form.useAiGeneratedContent
    ...
    : form.resumeText.trim().length >= 80;
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!canSubmit) {
      setError(
        !form.useAiGeneratedContent && resumePdf && form.resumeText.trim().length < 80
          ? "Paste at least 80 characters of resume text for a no-AI draft, or turn on Use AI-generated content."
          : "Add a PDF or at least 80 characters of resume text, plus at least 80 characters of job description.",
      );
      return;
    }
        ...
        <div className="grid gap-7 lg:grid-cols-[minmax(0,0.82fr)_minmax(500px,1.18fr)] lg:gap-20">
          <form onSubmit={handleSubmit} className="animate-rise-in rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card)/0.8)] p-5 shadow-[0_18px_50px_-35px_hsl(var(--primary)/0.45)] sm:p-7" style={{ animationDelay: "100ms" }} data-testid="form-cover-letter-input">
            ...
            <div className="space-y-6">
              <div>
                <div className="mb-2 flex items-center justify-between"><FieldLabel htmlFor="resume-text">Your resume</FieldLabel>{resumePdf ? <span className={`font-mono text-[10px] ${form.useAiGeneratedContent ? "text-[hsl(var(--accent-foreground))]" : "text-[hsl(var(--destructive))]"}`}>{form.useAiGeneratedContent ? "PDF ready for AI" : "Paste text for no-AI draft"}</span> : <TextCount value={form.resumeText} minimum={80} />}</div>
              ...
              <label htmlFor="use-ai-generated-content" className="flex cursor-pointer items-start gap-3 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--secondary)/0.45)] p-3.5 transition-colors hover:bg-[hsl(var(--secondary))]" data-testid="control-use-ai-generated-content">
                <input
                  id="use-ai-generated-content"
                  type="checkbox"
                  checked={form.useAiGeneratedContent}
                  onChange={(event) => updateField("useAiGeneratedContent", event.target.checked)}
                  className="mt-0.5 h-4 w-4 accent-[hsl(var(--primary))]"
                  data-testid="checkbox-use-ai-generated-content"
                />
                <span>
                  <span className="block text-sm font-semibold">Use AI-generated content</span>
                  <span className="mt-0.5 block text-xs leading-5 text-[hsl(var(--muted-foreground))]">
                    {form.useAiGeneratedContent
                      ? "OpenAI will tailor the draft from your resume and role."
                      : "Use the no-AI evidence-based draft from pasted text."}
                  </span>
                </span>
              </label>
            </div>
